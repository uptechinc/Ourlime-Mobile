...
describe('SharedContentMessageService', () => {
  ...
  test('keeps post and community links inside the native application', () => {
    ...
    const pluralPost = sharedContentMessageService.parse('https://ourlime.com/posts/post-2');
    const queryPost = sharedContentMessageService.parse('https://ourlime.com/post?id=post-3');
    ...
    expect(pluralPost?.mobileRoute).toBe('/post/post-2');
    expect(queryPost?.mobileRoute).toBe('/post/post-3');
    expect(community?.mobileRoute).toBe('/communities/community-1');
    expect(post?.visibleText).toBe('');
    expect(pluralPost?.visibleText).toBe('');
    expect(queryPost?.visibleText).toBe('');
