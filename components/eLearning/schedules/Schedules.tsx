...
export default function Schedules() {
	...
	const determineStatus = (schedule: Schedule): 'upcoming' | 'passed' => {
		...
		if (
			dayIndex > todayIndex ||
			(dayIndex === todayIndex && scheduleHour > currentHour)
		) {
			return 'upcoming';
		}
		return 'passed';
				...
				<View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
					...
					>
						...
						{schedule.isRecurring && (
							<Text
								style={{ fontSize: 8, fontStyle: 'italic', color: '#6b7280' }}
							>
		...
		<View style={{ flex: 1, backgroundColor: '#ffffff' }}>
			...
			>
				<SwipeDismissSurface
					...
					handleColor="#d1d5db"
					disabled={loading}
					accessibilityLabel="Swipe down to close schedule editor"
					style={{
						flex: 1,
						backgroundColor: '#ffffff',
						padding: 16,
										...
										>
											...
											<Text
												style={{
													...
													transform: [
														{
															rotate: formData.showStartTimeDropdown
																? '180deg'
																: '0deg',
														},
													],
																...
																style={{
																	...
																	color:
																		formData.startTime === time
																			...
																			: '#374151',
																	fontWeight:
																		formData.startTime === time ? '600' : '400',
																}}
															>
																{time}
															</Text>
											...
											onPress={() => {
												if (!formData.startTime) {
													Alert.alert(
														...
														'Please select a start time before choosing an end time.'
													);
													return;
												}
												setFormData({
													...formData,
													showEndTimeDropdown: !formData.showEndTimeDropdown,
												});
											}}
											style={{
												borderWidth: 1,
										...
										>
											...
											<Text
												style={{
													...
													transform: [
														{
															rotate: formData.showEndTimeDropdown
																? '180deg'
																: '0deg',
														},
													],
											...
											>
												<ScrollView showsVerticalScrollIndicator={false}>
													{timeSlots
														.filter(
															(time) =>
																parseInt(time) > parseInt(formData.startTime)
														)
														.map((time) => (
															<TouchableOpacity
																key={time}
										...
										{daysOfWeek.map((day) => (
											<TouchableOpacity
												...
												style={{
													...
													borderRadius: 6,
													backgroundColor:
														formData.day === day ? '#10b981' : '#f3f4f6',
												}}
											>
												<Text
													style={{
			...
			>
				<SwipeDismissSurface
					...
					handleColor="#d1d5db"
					accessibilityLabel="Swipe down to close schedule template"
					style={{
						flex: 1,
						backgroundColor: '#ffffff',
						padding: 16,
					...
					<View style={{ gap: 16 }}>
						...
						<View style={{ flexDirection: 'row', gap: 8 }}>
							...
							<TouchableOpacity
								...
								disabled={
									selectedSchedules.length === 0 || !templateName.trim()
								}
							>
								<Text
									style={{
			...
			>
				<SwipeDismissSurface
					visible={showTemplateListModal}
					onDismiss={() => setShowTemplateListModal(false)}
					handleColor="#d1d5db"
					accessibilityLabel="Swipe down to close saved templates"
					style={{
						flex: 1,
						backgroundColor: '#ffffff',
						padding: 16,
				...
				>
					...
					>
						<Text style={{ fontSize: 16, color: '#374151' }}>Close</Text>
					</TouchableOpacity>
				</SwipeDismissSurface>
			</Modal>
		</View>
