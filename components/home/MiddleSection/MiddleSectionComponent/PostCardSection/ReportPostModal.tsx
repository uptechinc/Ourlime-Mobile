...
export default function ReportPostModal({ visible, post, onClose }: ReportPostModalProps) {
  ...
  const swipeDismiss = useSwipeDismiss({ visible, onDismiss: handleClose, disabled: submitting });
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      if (post.origin === 'community' && post.communityId && category !== CHILD_SAFETY_REASON_CATEGORY) {
        ...
        await moderationService.reportPost({
          ...
          evidenceFiles: category === CHILD_SAFETY_REASON_CATEGORY ? [] : evidenceFiles,
        });
      }
      setSubmitting(false);
      handleClose();
  ...
  return (
    ...
    <Modal visible={visible} transparent statusBarTranslucent navigationBarTranslucent presentationStyle="overFullScreen" animationType="none" onRequestClose={swipeDismiss.dismissWithAnimation}>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.canvas }} edges={['top', 'left', 'right']}>
        <SwipeDismissHandle gesture={swipeDismiss.gesture} color={colors.border} animatedStyle={swipeDismiss.handleAnimatedStyle} accessibilityLabel="Swipe down to close post report" />
        ...
        <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }}>
          <View style={{ padding: 13, borderRadius: 14, backgroundColor: colors.control, marginBottom: 18 }}><Text numberOfLines={3} style={{ color: colors.secondaryText }}>{linkPresentationService.compactUrlsInText(post.caption || post.description || 'Post content')}</Text></View>
          {!category ? (
            <View>
              ...
              {(Object.entries(REPORT_REASONS) as [ReportReasonCategory, (typeof REPORT_REASONS)[ReportReasonCategory]][]).map(([key, group]) => (
                <TouchableOpacity key={key} onPress={() => { setCategory(key); if (key === CHILD_SAFETY_REASON_CATEGORY) setEvidenceFiles([]); }} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10, padding: 15, borderRadius: 15, borderWidth: 1, borderColor: key === CHILD_SAFETY_REASON_CATEGORY ? colors.destructive : colors.border, backgroundColor: key === CHILD_SAFETY_REASON_CATEGORY ? colors.destructiveSurface : colors.surface }}><Icon name={key === CHILD_SAFETY_REASON_CATEGORY ? 'shield' : 'alert-circle'} size={19} color={colors.destructive} /><Text style={{ flex: 1, marginLeft: 11, color: key === CHILD_SAFETY_REASON_CATEGORY ? colors.destructiveText : colors.secondaryText, fontWeight: '700' }}>{group.label}</Text><Icon name="chevron-right" size={19} color={colors.icon} /></TouchableOpacity>
              ))}
            </View>
          ) : (
            <View>
              ...
              {reason && category === CHILD_SAFETY_REASON_CATEGORY ? (
                <View style={{ marginTop: 14, padding: 14, borderRadius: 14, borderWidth: 1, borderColor: colors.destructive, backgroundColor: colors.destructiveSurface }}>
                  <Text style={{ color: colors.destructiveText, fontWeight: '900' }}>Do not attach suspected CSAM.</Text>
                  <Text style={{ marginTop: 6, color: colors.destructiveText, lineHeight: 19 }}>Do not download, copy, forward, email, upload, or redistribute suspected child sexual abuse material. The original item identifier is included securely with this report.</Text>
                </View>
              ) : reason ? (
