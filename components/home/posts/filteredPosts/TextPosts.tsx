...
export default function TextPosts({ socialPosts }: { socialPosts: SocialPosts[] }) {
  // Only posts with non-empty content
  const textPosts = socialPosts.filter((post) => post.content.trim() !== '');

  return (
    <ScrollView>
      {textPosts.length === 0 ? (
        <Text style={{ textAlign: 'center', fontSize: 16, marginVertical: 16 }}>No text posts found.</Text>
      ) : (
