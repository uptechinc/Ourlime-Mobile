...
export default function PhotosPosts({ socialPosts }: { socialPosts: SocialPosts[] }) {
  // Only posts with a postImage
  ...
  return (
    <ScrollView>
      {photosPosts.length === 0 ? (
        <Text style={{ textAlign: 'center', fontSize: 16, marginVertical: 16 }}>No photo posts found.</Text>
      ) : (
        photosPosts.map((post, index) => (
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
              ...
              <Text style={{ fontSize: 20, fontWeight: '600' }}>{post.username}</Text>
              <Text style={{ marginLeft: 'auto', fontSize: 15, color: '#888' }}>{formatDate(post.time)}</Text>
            </View>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 8 }}>
                ...
                {post.postImage && (
                  <Image
                    ...
                    style={{ marginTop: 12, height: 220, width: '100%', resizeMode: 'cover', borderRadius: 8 }}
                  />
                )}
              </View>
