...
export default function VideosPosts({ socialPosts }: { socialPosts: SocialPosts[] }) {
  ...
  return (
    <ScrollView>
      {videosPosts.length === 0 ? (
        <Text style={{ textAlign: 'center', fontSize: 16, marginVertical: 16 }}>No video posts found.</Text>
      ) : (
        videosPosts.map((post, index) => (
          <View
            key={index}
            style={{
              marginBottom: 20,
