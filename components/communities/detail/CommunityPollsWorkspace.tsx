...
export default function CommunityPollsWorkspace({
	resource,
	canCreate,
	createRequestKey,
	onRetry,
	...
	const handleCreate = async (): Promise<void> => {
		...
		if (
			...
			setFeedback(
				'Enter a question, two to five complete options, and a valid duration.'
			);
			return;
		}
		setBusy(true);
		try {
			...
			setFeedback(
				error instanceof Error ? error.message : 'Poll could not be created.'
			);
		} finally {
			setBusy(false);
		}
		...
		return (
			<View style={{ padding: 28, alignItems: 'center' }}>
				<Text style={{ color: colors.destructiveText, textAlign: 'center' }}>
					{resource.error?.message ?? 'Polls could not be loaded.'}
				</Text>
				<TouchableOpacity
					onPress={onRetry}
					style={{
		...
		<View style={{ margin: 16 }}>
			...
			>
				...
				{canCreate ? (
					...
					>
						<Plus size={15} color={colors.onAccent} />
						<Text
							style={{
								marginLeft: 4,
			...
			{resource.data?.length ? (
				resource.data.map((poll) => (
					...
					>
						<View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
							...
							<TouchableOpacity
								onPress={() => onReport(poll)}
								style={{ padding: 6 }}
							>
								<Flag size={16} color={colors.icon} />
							</TouchableOpacity>
						...
						<View style={{ marginTop: 14 }}>
							{poll.options.map((option, optionIndex) => {
								...
								return (
									<TouchableOpacity
										key={option.id}
										disabled={!poll.permissions.canVote || busy}
										onPress={() => {
											setBusy(true);
											void onVote(poll.id, optionIndex)
												.catch((error: unknown) =>
													setFeedback(
										...
										style={{
											...
											overflow: 'hidden',
											borderWidth: 1,
											borderColor: option.selectedByViewer
												? colors.accent
												: colors.border,
									...
									>
										<View
											style={{
												...
												bottom: 0,
												width: `${percentage}%`,
												backgroundColor: option.selectedByViewer
													? '#6ee7b7'
													: colors.disabled,
											}}
										/>
						...
						<Text style={{ color: colors.mutedText, fontSize: 11 }}>
							{poll.totalVotes} total votes · ends{' '}
				...
				<View style={{ alignItems: 'center', padding: 30 }}>
					<BarChart3 size={40} color={colors.accent} />
					<Text
						style={{ marginTop: 10, color: colors.text, fontWeight: '900' }}
					>
						No polls yet
					</Text>
					<Text style={{ marginTop: 4, color: colors.mutedText }}>
						Ask the community its first question.
					</Text>
				</View>
			)}
			<Modal
			...
			>
				<SwipeDismissSurface
					...
					disabled={busy}
					accessibilityLabel="Swipe down to close poll creation"
				...
				>
					...
					>
						...
						>
							<CheckSquare size={19} color={colors.accent} />
							<View style={{ flex: 1, marginLeft: 9 }}>
								...
								>
									Members can choose more than one answer.
								</Text>
							</View>
