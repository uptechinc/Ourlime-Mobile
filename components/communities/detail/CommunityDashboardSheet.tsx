...
export default function CommunityDashboardSheet({
	...
	const activities = useMemo(() => {
		const query = search.trim().toLowerCase();
		return (dashboard.data?.activities ?? []).filter(
			(activity) =>
				!query ||
				`${activity.title} ${activity.creatorName} ${activity.type}`
					.toLowerCase()
					.includes(query)
		);
	...
	const reports = useMemo(() => {
		const query = search.trim().toLowerCase();
		return (dashboard.data?.reports ?? []).filter(
			(report) =>
				(statusFilter === 'all' || report.status === statusFilter) &&
				(!query ||
					`${report.title} ${report.reporterName} ${report.reason}`
						.toLowerCase()
						.includes(query))
		);
	...
	const renderOverview = () => {
		...
		const metrics = [
			{
				label: 'Members',
				value: data?.memberCount ?? community.memberCount,
				icon: Users,
			},
			{
				label: 'Posts',
				value: data?.postCount ?? community.postCount,
				icon: BarChart3,
			},
		...
		return (
			...
			>
				{metrics.map((metric) => (
					<View key={metric.label} style={{ width: '50%', padding: 5 }}>
						<View
							style={{
								padding: 16,
								borderRadius: 17,
						...
						>
							<metric.icon size={20} color={colors.accent} />
							<Text
								style={{
									marginTop: 10,
									color: colors.text,
	...
	const dashboardTab =
		tab === 'overview' || tab === 'activity' || tab === 'reports';
	const dashboardFailed =
		dashboardTab && dashboard.status === 'error' && !dashboard.data;
	...
	return (
		...
		>
			<SwipeDismissSurface
				...
				accessibilityLabel="Swipe down to close community dashboard"
				...
				>
					...
					>
						<View style={{ flex: 1 }}>
							...
							>
								{community.title} · {community.viewerRole}
							</Text>
						</View>
						<TouchableOpacity
							onPress={onClose}
							accessibilityLabel="Close community dashboard"
					...
					<ScrollView
						keyboardShouldPersistTaps="handled"
						contentContainerStyle={{ padding: 16, paddingBottom: 45 }}
					>
						<View
							style={{
								flexDirection: 'row',
						...
						>
							...
							>
								{TABS.find((option) => option.value === tab)?.label}
							</Text>
							<TouchableOpacity
							...
							<>
								...
								{!members.data ? (
									members.status === 'error' ? (
										renderWorkspaceError(
											members.error?.message ?? 'Members could not be loaded.',
											onLoadMembers
										)
									) : (
										<ActivityIndicator
										...
										>
											{member.profilePicture ? (
												<CachedImage
													...
													recyclingKey={`dashboard-member-${member.userId}-${member.profilePicture}`}
											...
											<Shield size={17} color={colors.icon} />
										</TouchableOpacity>
									))
								)}
							</>
