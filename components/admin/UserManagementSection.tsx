...
export default function UserManagementSection() {
  ...
  return <>
    ...
    <Text style={{ marginVertical: 10, color: '#64748b', fontSize: 12 }}>Showing {visibleUsers.length} of {filteredUsers.length} users</Text>
    {visibleUsers.map((user) => <TouchableOpacity key={user.id} onPress={() => { setSelectedUser(user); setDetailTab('overview'); setNewStatus(user.accountStatus === 'active' || user.accountStatus === 'pending' || user.accountStatus === 'suspended' || user.accountStatus === 'banned' ? user.accountStatus : 'active'); setStatusReason(user.statusReason); setDeleteConfirmation(''); }} style={{ flexDirection: 'row', alignItems: 'center', padding: 13, marginBottom: 9, borderRadius: 15, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e2e8f0' }}><UserAvatar profileImage={user.profilePicture} firstName={user.firstName || user.userName || user.email} size={44} /><View style={{ flex: 1, marginLeft: 11 }}><Text numberOfLines={1} style={{ fontWeight: '900', color: '#0f172a' }}>{`${user.firstName} ${user.lastName}`.trim() || user.userName || user.email}</Text><Text numberOfLines={1} style={{ marginTop: 2, color: '#64748b', fontSize: 12 }}>@{user.userName || 'unknown'} · {user.role} · {user.accountType}</Text><Text style={{ marginTop: 4, color: user.archived || user.banned ? '#b91c1c' : '#047857', fontSize: 10, fontWeight: '900', textTransform: 'uppercase' }}>{user.archived ? 'archived' : user.accountStatus}</Text></View><Icon name="chevron-right" size={18} color="#94a3b8" /></TouchableOpacity>)}
    {pageCount > 1 ? <View style={{ marginTop: 8, flexDirection: 'row', alignItems: 'center' }}><TouchableOpacity disabled={page <= 1} onPress={() => setPage((current) => Math.max(1, current - 1))} style={{ padding: 10 }}><Icon name="chevron-left" size={20} color={page <= 1 ? '#cbd5e1' : '#047857'} /></TouchableOpacity><Text style={{ flex: 1, textAlign: 'center', color: '#64748b' }}>Page {Math.min(page, pageCount)} of {pageCount}</Text><TouchableOpacity disabled={page >= pageCount} onPress={() => setPage((current) => Math.min(pageCount, current + 1))} style={{ padding: 10 }}><Icon name="chevron-right" size={20} color={page >= pageCount ? '#cbd5e1' : '#047857'} /></TouchableOpacity></View> : null}
    <Modal visible={Boolean(selectedUser)} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setSelectedUser(null)}><SafeAreaView edges={['top', 'left', 'right']} style={{ flex: 1, backgroundColor: '#f8fafc' }}><View style={{ flexDirection: 'row', alignItems: 'center', padding: 16, backgroundColor: '#fff' }}><Text style={{ flex: 1, fontSize: 19, fontWeight: '900' }}>Manage user</Text><TouchableOpacity onPress={() => setSelectedUser(null)}><Icon name="x" size={24} color="#475569" /></TouchableOpacity></View><ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexGrow: 0, backgroundColor: '#fff', paddingHorizontal: 12 }}>{(['overview', 'role', 'status', 'verification', 'lifecycle'] as const).map((tab) => <TouchableOpacity key={tab} onPress={() => setDetailTab(tab)} style={{ marginRight: 7, marginBottom: 10, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: detailTab === tab ? '#10b981' : '#e2e8f0' }}><Text style={{ textTransform: 'capitalize', color: detailTab === tab ? '#fff' : '#475569', fontSize: 12, fontWeight: '800' }}>{tab}</Text></TouchableOpacity>)}</ScrollView>{selectedUser ? <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 55 }}>
      {detailTab === 'overview' ? <><View style={{ alignItems: 'center' }}><UserAvatar profileImage={selectedUser.profilePicture} firstName={selectedUser.firstName || selectedUser.userName} size={72} /><Text style={{ marginTop: 10, fontSize: 20, fontWeight: '900' }}>{`${selectedUser.firstName} ${selectedUser.lastName}`.trim()}</Text><Text style={{ color: '#64748b' }}>{selectedUser.email}</Text></View>{[['Username', `@${selectedUser.userName || 'unknown'}`], ['Account', selectedUser.accountType], ['Role', selectedUser.role], ['Status', selectedUser.archived ? 'archived' : selectedUser.accountStatus], ['Email verified', selectedUser.emailVerified ? 'Yes' : 'No'], ['Identity verified', selectedUser.isAuthenticated ? 'Yes' : selectedUser.verificationStatus], ['Online status', selectedUser.onlineStatus]].map(([label, value]) => <View key={label} style={{ marginTop: 11, padding: 13, borderRadius: 13, backgroundColor: '#fff' }}><Text style={{ color: '#64748b', fontSize: 11, fontWeight: '800' }}>{label}</Text><Text style={{ marginTop: 3, color: '#0f172a', fontWeight: '800', textTransform: 'capitalize' }}>{value}</Text></View>)}</> : null}
      {detailTab === 'role' ? <><Text style={{ marginBottom: 10, color: '#334155', fontWeight: '900' }}>Platform role</Text>{ROLES.map((role) => <TouchableOpacity key={role} disabled={busy || selectedUser.role === role || selectedUser.id === adminUserService.getCurrentUserId()} onPress={() => void handleRole(role)} style={{ flexDirection: 'row', alignItems: 'center', padding: 14, marginBottom: 7, borderRadius: 13, backgroundColor: selectedUser.role === role ? '#d1fae5' : '#fff' }}><View style={{ flex: 1 }}><Text style={{ textTransform: 'capitalize', color: selectedUser.role === role ? '#047857' : '#334155', fontWeight: '900' }}>{role}</Text><Text style={{ marginTop: 2, color: '#64748b', fontSize: 11 }}>{role === 'admin' ? 'Full administration access' : role === 'moderator' ? 'Content reports and moderation' : role === 'developer' ? 'Developer previews and tools' : role === 'premium' ? 'Premium product capabilities' : 'Standard member access'}</Text></View>{selectedUser.role === role ? <Icon name="check" size={18} color="#047857" /> : null}</TouchableOpacity>)}</> : null}
      {detailTab === 'status' ? <><Text style={{ marginBottom: 10, color: '#334155', fontWeight: '900' }}>Account access status</Text>{(['active', 'pending', 'suspended', 'banned'] as const).map((status) => <TouchableOpacity key={status} onPress={() => setNewStatus(status)} style={{ marginBottom: 7, padding: 13, borderRadius: 13, backgroundColor: newStatus === status ? '#d1fae5' : '#fff' }}><Text style={{ color: newStatus === status ? '#047857' : '#334155', textTransform: 'capitalize', fontWeight: '900' }}>{status}</Text></TouchableOpacity>)}{newStatus === 'suspended' ? <TextInput value={suspensionDays} onChangeText={setSuspensionDays} keyboardType="number-pad" placeholder="Suspension days" style={{ marginTop: 7, borderRadius: 13, borderWidth: 1, borderColor: '#e2e8f0', padding: 12, backgroundColor: '#fff' }} /> : null}<TextInput value={statusReason} onChangeText={setStatusReason} multiline placeholder="Administrative reason shown to the user" style={{ minHeight: 86, marginTop: 9, textAlignVertical: 'top', borderRadius: 13, borderWidth: 1, borderColor: '#e2e8f0', padding: 12, backgroundColor: '#fff' }} /><TouchableOpacity disabled={busy || selectedUser.id === adminUserService.getCurrentUserId()} onPress={() => void handleStatus()} style={{ marginTop: 12, alignItems: 'center', borderRadius: 14, backgroundColor: newStatus === 'banned' ? '#c64d53' : '#10b981', padding: 13 }}><Text style={{ color: '#fff', fontWeight: '900' }}>Apply status</Text></TouchableOpacity></> : null}
