...
export default function GalleryTab({ userId }: GalleryTabProps) {
  ...
  if (resource.error && mediaList.length === 0) return <View style={{ paddingVertical: 40, alignItems: 'center', paddingHorizontal: 24 }}><Text style={{ color: colors.destructiveText, textAlign: 'center' }}>{resource.error.message}</Text><TouchableOpacity onPress={() => void refresh()} style={{ marginTop: 12, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 18, backgroundColor: colors.accent }}><Text style={{ color: colors.onAccent, fontWeight: '700' }}>Retry</Text></TouchableOpacity></View>;
  if (mediaList.length === 0) return <View style={{ paddingVertical: 40, alignItems: 'center' }}><Ionicons name="images-outline" size={40} color={colors.icon} /><Text style={{ marginTop: 10, fontSize: 14, color: colors.mutedText, fontWeight: '500' }}>No photos or videos shared yet</Text></View>;
  ...
  return (
    <View style={{ padding: 16, flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
      ...
      {resource.data?.hasMore ? <TouchableOpacity onPress={() => void loadMore()} style={{ width: '100%', alignItems: 'center', paddingVertical: 12 }}><Text style={{ color: colors.accentText, fontWeight: '700' }}>Load more media</Text></TouchableOpacity> : null}
    </View>
  );
}
