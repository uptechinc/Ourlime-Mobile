...
export default function Blogs() {
    ...
    const filteredBlogs = blogs.filter(blog => {
        const matchesSearch = searchQuery === '' ||
            ...
            (blog.tags && blog.tags.some((tag: any) => tag.name?.toLowerCase().includes(searchQuery.toLowerCase())));

        const matchesCategory = selectedCategory === 'All' || blog.category === selectedCategory || (blog.categories && blog.categories.some((cat: any) => cat.name === selectedCategory));
    ...
    const sortedBlogs = [...filteredBlogs].sort((a, b) => {
        if (sortBy === 'newest') {
            ...
            return (b.engagement?.[0]?.likesCount || 0) - (a.engagement?.[0]?.likesCount || 0);
        } else if (sortBy === 'comments') {
            return (b.engagement?.[0]?.commentsCount || 0) - (a.engagement?.[0]?.commentsCount || 0);
        }
        return 0;
    });
                                                ...
                                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
                                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                        ...
                                                        <Text style={{ color: '#6b7280', fontSize: 12 }}>
                                                            {blog.engagement?.[0]?.likesCount || 0}
                                                        </Text>
                                                    </View>
                                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                        <Text style={{ color: '#6b7280', fontSize: 14, marginRight: 4 }}>💬</Text>
                                                        <Text style={{ color: '#6b7280', fontSize: 12 }}>
                                                            {blog.engagement?.[0]?.commentsCount || 0}
                                                        </Text>
                                                    </View>
                                                </View>
                                            </View>
