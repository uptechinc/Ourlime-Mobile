...
const EMPTY_DASHBOARD: ResourceState<CommunityDashboardData> = {
	data: null,
	status: 'idle',
	source: 'memory',
	updatedAt: null,
...
export default function CommunityDetailScreen() {
	...
	const params = useLocalSearchParams<{ id: string; dashboard?: string }>();
	...
	const { resource: postResource, refresh: refreshPosts } =
		useCommunityFeedResource(
			viewerId,
			communityId,
			Boolean(community?.permissions.canView)
		);
	const posts = useMemo(() => postResource.data ?? [], [postResource.data]);
	const categoriesData = useResourceStore(
		(state) => state.communityCategories.data
	);
	const categories = useMemo(() => categoriesData ?? [], [categoriesData]);
	const dashboard =
		useResourceStore((state) =>
			communityId ? state.communityDashboards[communityId] : undefined
		) ?? EMPTY_DASHBOARD;
	...
	const [managedMember, setManagedMember] = useState<CommunityMember | null>(
		null
	);
	...
	const dashboardTarget =
		params.dashboard === 'requests' || params.dashboard === 'reports'
			? params.dashboard
			: undefined;
	...
	useEffect(() => {
		if (
			!communityId ||
			!dashboardTarget ||
			handledDashboardTargetRef.current === `${communityId}:${dashboardTarget}`
		)
			return;
		handledDashboardTargetRef.current = `${communityId}:${dashboardTarget}`;
		setDashboardVisible(true);
	}, [communityId, dashboardTarget]);
	...
	const reconcileCommunity = useCallback(async (): Promise<void> => {
		...
		await refreshDetail();
		const latest =
			useResourceStore.getState().communityDetails[identifier]?.data?.community;
	...
	const handleMembershipAction = async (): Promise<void> => {
		...
		try {
			...
			if (community.membershipState === 'pending') {
				...
				setFeedback(
					result.membershipState === 'member'
						...
						: 'Your request was sent to the community team.'
				);
			}
			...
			const state = {
				...detail,
				data: detailData ? { ...detailData, community: updated } : null,
			...
			if (community.id && community.id !== identifier)
				useResourceStore.getState().setCommunityDetail(community.id, state);
	...
	const handleLike = async (): Promise<void> => {
		...
		setBusy(true);
		try {
			const reaction = await communityService.toggleCommunityLike(
				community.id,
				!community.isLikedByViewer
			);
			const updated: CommunityCardModel = {
				...community,
				isLikedByViewer: reaction.liked,
				likeCount: reaction.likeCount,
			};
			useResourceStore
				.getState()
				.setCommunityDetail(identifier, {
					...detail,
					data: detailData ? { ...detailData, community: updated } : null,
					status: 'ready',
					source: 'network',
			...
			setFeedback(
				error instanceof Error
					? error.message
					: 'Community like could not be updated.'
			);
		} finally {
			setBusy(false);
		}
	...
	const handlePostUpdate = (post: PostItem): void => {
		void feedService.patchPost(post);
	};
	const handlePostDelete = (postId: string): void => {
		void feedService.removePosts((post) => post.id === postId);
	};
	...
	const handleCreateEvent = async (
		...
		if (!community || !profile)
			throw new Error('Your verified profile is still loading.');
	...
	const handleReviewRequest = async (
		...
		if (!community) return;
		await communityService.reviewJoinRequest(
			community.id,
			request.requestId,
			request.userId,
			action
	...
	const handleLoadDashboard = useCallback(
		(force = false): void => {
			...
			void dashboardService
				.hydrate(viewerId, communityId)
				.then(() => dashboardService.refresh(viewerId, communityId, force));
		},
		[communityId, viewerId]
	);

	const handleLoadDashboardMembers = useCallback(
		(force = false): void => {
	...
	const handleLoadDashboardRequests = useCallback(
		(force = false): void => {
	...
	const handleModerateReport = async (
		...
		if (!community) return;
		await dashboardService.moderate(viewerId, {
			...
			resolutionNote:
				action === 'hide'
					? 'Content hidden by a community moderator.'
					: undefined,
		});
		if (action === 'hide' && report.targetType === 'post') await refreshPosts();
		if (action === 'hide' && report.targetType === 'event')
			await loadEvents(true);
		if (action === 'hide' && report.targetType === 'poll')
	...
	const handleConfirm = async (): Promise<void> => {
		...
		try {
			if (confirmation.action === 'leave') {
				...
				await detailService.deletePoll(
					viewerId,
					community.id,
					confirmation.poll.id
				);
			}
			setConfirmation(INITIAL_CONFIRMATION);
		} catch (error: unknown) {
	...
	const handleSubmitReport = async (
		...
		if (category === CHILD_SAFETY_REASON_CATEGORY) {
			const targetId =
				reportTarget.kind === 'community'
					? community.id
					: reportTarget.kind === 'poll'
						? reportTarget.poll.id
						: reportTarget.event.id;
			if (!targetId)
				throw new Error(
					'The selected community content could not be identified.'
				);
			const reference = await moderationService.reportContent(
				reportTarget.kind === 'event'
					? 'event'
					: reportTarget.kind === 'poll'
						? 'other'
						: 'community',
				{
					...
					reason,
					description: details,
					routePath: `/communities/${community.slug || community.id}`,
		...
		if (reportTarget.kind === 'community') {
			await moderationService.reportCommunity({
	...
	const initialLoading =
		!detail.data && (detail.status === 'idle' || detail.status === 'hydrating');
	const errorMessage =
		detail.error?.message ?? 'This community could not be loaded.';
		...
		>
			...
			>
				<TouchableOpacity
					...
					style={{ padding: 8 }}
				>
					<ChevronLeft size={27} color={colors.icon} />
				</TouchableOpacity>
				<Text
					numberOfLines={1}
					style={{
				...
				{community ? (
					<>
						...
						{community.permissions.canReport ? (
							<TouchableOpacity
								onPress={() => setReportTarget({ kind: 'community' })}
			...
			{initialLoading ? (
				...
				<ScrollView
					refreshControl={
						<RefreshControl
							refreshing={detail.status === 'refreshing'}
				...
				>
					<CommunityDetailHeader
						community={community}
						busyAction={busy}
						onMembershipAction={() => void handleMembershipAction()}
						onLeave={() =>
							showConfirmation(
								'leave',
								'Leave community?',
								`You will stop seeing member-only content from ${community.title}.`
							)
						}
					...
					{!community.permissions.canView ? (
						...
						>
							<Lock size={38} color={colors.icon} />
							<Text
								style={{
									marginTop: 11,
									color: colors.text,
							...
							>
								An approved membership is required to view this community’s
								workspaces.
							</Text>
						</View>
					) : (
						<>
							...
							community.permissions.canHostEvent ||
							community.permissions.canCreatePoll ||
								...
								>
									...
									{community.permissions.canHostEvent ? (
										<TouchableOpacity
											onPress={() => {
												setActiveTab('events');
												setEventCreateRequestKey((current) => current + 1);
											}}
											style={{
												marginHorizontal: 4,
												paddingHorizontal: 13,
									...
									{community.permissions.canCreatePoll ? (
										<TouchableOpacity
											onPress={() => {
												setActiveTab('polls');
												setPollCreateRequestKey((current) => current + 1);
											}}
											style={{
												marginHorizontal: 4,
												paddingHorizontal: 13,
							...
							{activeTab === 'events' ? (
								<CommunityEventsWorkspace
									resource={events}
									canCreate={community.permissions.canHostEvent}
									createRequestKey={eventCreateRequestKey}
									onRetry={() => void loadEvents(true)}
									onCreate={handleCreateEvent}
									onToggleAttendance={async (eventId) => {
										await eventService.toggleCommunityAttendance(
											community.id,
											eventId
										);
										await loadEvents(true);
									}}
									onUpdate={async (eventId, draft) => {
										await eventService.updateCommunityEvent(
											community.id,
											eventId,
											{
												title: draft.title,
									...
									onDelete={async (eventId) => {
										await eventService.deleteCommunityEvent(
											community.id,
											eventId
										);
										await loadEvents(true);
									...
									onReport={(event) =>
										setReportTarget({ kind: 'event', event })
									}
								/>
							) : null}
							{activeTab === 'polls' ? (
								<CommunityPollsWorkspace
									resource={polls}
									canCreate={community.permissions.canCreatePoll}
									createRequestKey={pollCreateRequestKey}
									onRetry={() => void loadPolls(true)}
									onCreate={async (
										question,
										options,
										durationHours,
										...
										detailService.createPoll(viewerId, {
											communityId: community.id,
											question,
											options,
											durationHours,
									...
									onDelete={(poll) =>
										showConfirmation(
											'poll-delete',
											'Delete poll?',
											'This poll and its votes will be permanently removed.',
											poll
										)
									}
									onReport={(poll) => setReportTarget({ kind: 'poll', poll })}
								/>
							) : null}
							{activeTab === 'about' ? (
								<CommunityAboutWorkspace detail={detailData} />
							) : null}
							{activeTab === 'members' ? (
								<CommunityMembersWorkspace
									resource={members}
									canManage={community.permissions.canManageMembers}
									onRetry={() => void loadMembers(true)}
									onLoadMore={() => void loadMoreMembers()}
									onSearch={(search) => void searchMembers(search)}
									onOpenProfile={(member) =>
										router.push({
											pathname: '/profile/[username]',
											params: { username: member.userName || member.userId },
										})
									}
			...
			{createPostVisible && profile && community ? (
				<CreatePostModal
					setTogglePostForm={setCreatePostVisible}
					userProfile={profile}
					communityId={community.id}
			...
			{community ? (
				<CommunityDashboardSheet
					visible={dashboardVisible}
					initialTab={dashboardTarget}
					community={community}
					members={members}
					requests={requests}
					dashboard={dashboard}
				...
				<EditCommunityModal
					...
					onDelete={() => {
						...
						showConfirmation(
							'delete',
							'Delete community?',
							`${community.title} and its community data will be permanently removed.`
						);
					}}
				/>
			) : null}
			<CommunityMemberActionSheet
				visible={Boolean(managedMember)}
			...
			<CustomModal
				...
				type={confirmation.action === 'leave' ? 'warning' : 'danger'}
				confirmText={
					confirmation.action === 'leave' ? 'Leave community' : 'Delete'
				}
				cancelText="Cancel"
				isLoading={busy}
				onConfirm={() => void handleConfirm()}
				onCancel={() => setConfirmation(INITIAL_CONFIRMATION)}
				onClose={() => setConfirmation(INITIAL_CONFIRMATION)}
			/>
			<CommunityReportModal
				...
				subjectLabel={
					reportTarget?.kind === 'poll'
						? reportTarget.poll.question
						: reportTarget?.kind === 'event'
							? reportTarget.event.title
							: (community?.title ?? '')
				}
				childSafetyTarget={
					community && reportTarget
						? {
								...
								id:
									reportTarget.kind === 'event'
										? reportTarget.event.id || community.id
										: reportTarget.kind === 'poll'
											? reportTarget.poll.id || community.id
											: community.id,
