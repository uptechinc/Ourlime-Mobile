...
export default function ProjectManagementScreen() {
  ...
  const invitations = projects.filter((project) => project.membershipStatus === 'pending');
  const acceptedProjects = projects.filter((project) => project.membershipStatus === 'accepted');
    ...
    <SafeAreaView edges={['top', 'left', 'right']} style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void loadProjects(true); }} tintColor="#10b981" />}>
        ...
        {!loading && invitations.length > 0 ? <View style={styles.section}>
          ...
          {invitations.map((project) => <View key={project.id} style={[styles.card, styles.inviteCard]}>
        ...
        {!loading ? <View style={styles.section}>
          ...
          {acceptedProjects.length === 0 ? <View style={styles.empty}><FolderOpen size={38} color={colors.mutedText} /><Text style={styles.emptyTitle}>No projects yet</Text><Text style={styles.cardText}>Create your first project or accept a team invitation.</Text></View> : acceptedProjects.map((project) => <View key={project.id} style={styles.card}>
