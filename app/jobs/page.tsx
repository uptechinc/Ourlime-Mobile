...
export default function JobsPage() {
  ...
  const [categories, setCategories] = useState<
    { name: string; count: number; icon: any }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (!search.trim()) {
      ...
      setFiltered(
        jobs.filter(
          (j) =>
            j.basic_info.title.toLowerCase().includes(q) ||
            j.basic_info.description.toLowerCase().includes(q) ||
            (j.category_specific.name?.toLowerCase() ?? '').includes(q) ||
            j.details.skills.some((s) => s.toLowerCase().includes(q))
        )
      );
    }
  }, [jobs, search]);
